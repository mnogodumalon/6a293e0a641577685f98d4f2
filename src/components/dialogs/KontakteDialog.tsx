...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Kontakte';
...
interface KontakteDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Kontakte['fields']) => Promise<void>;
  defaultValues?: Kontakte['fields'];
...
export function KontakteDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: KontakteDialogProps) {
  const [fields, setFields] = useState<Partial<Kontakte['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults((defaultValues ?? {}) as Record<string, unknown>, formEnhancements.defaults) as Partial<Kontakte['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'kontakte');
      await onSubmit(clean as Kontakte['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Kontakte['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Kontakte bearbeiten' : 'Kontakte hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
      ...
      <div key="kategorie" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kategorie) === 'privat'}
            onClick={() => setFields(f => ({ ...f, kategorie: (lookupKey(f.kategorie) === 'privat' ? undefined : 'privat') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kategorie) === 'privat'
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kategorie) === 'geschaeftlich'}
            onClick={() => setFields(f => ({ ...f, kategorie: (lookupKey(f.kategorie) === 'geschaeftlich' ? undefined : 'geschaeftlich') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kategorie) === 'geschaeftlich'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kategorie) === 'familie'}
            onClick={() => setFields(f => ({ ...f, kategorie: (lookupKey(f.kategorie) === 'familie' ? undefined : 'familie') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kategorie) === 'familie'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kategorie) === 'freunde'}
            onClick={() => setFields(f => ({ ...f, kategorie: (lookupKey(f.kategorie) === 'freunde' ? undefined : 'freunde') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kategorie) === 'freunde'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'mobil': (
      <div key="mobil" className="space-y-1.5">
    ...
    'notizen': (
      <div key="notizen" className="space-y-1.5">
        ...
        <Textarea
          id="notizen"
          placeholder="Notizen, Anmerkungen, besondere Informationen..."
  ...
  const FIELD_LABELS: Record<string, string> = {"nachname": "Nachname", "geburtsdatum": "Geburtsdatum", "kategorie": "Kategorie", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "mobil": "Mobilnummer", "webseite": "Webseite", "strasse": "Straße", "hausnummer": "Hausnummer", "postleitzahl": "Postleitzahl", "ort": "Ort", "notizen": "Notizen", "vorname": "Vorname"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.KONTAKTE} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
