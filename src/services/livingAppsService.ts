...
import type { Kontakte, CreateKontakte } from '@/types/app';
...
export class LivingAppsService {
  // --- KONTAKTE ---
  static async getKontakte(): Promise<Kontakte[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.KONTAKTE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Kontakte[];
    return enrichLookupFields(records, 'kontakte');
  }
  static async getKontakteEntry(id: string): Promise<Kontakte | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.KONTAKTE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Kontakte;
    return enrichLookupFields([record], 'kontakte')[0];
  }
  static async createKontakteEntry(fields: CreateKontakte) {
    return callApi('POST', `/apps/${APP_IDS.KONTAKTE}/records`, { fields: cleanFieldsForApi(fields as any, 'kontakte') });
  }
  static async updateKontakteEntry(id: string, fields: Partial<CreateKontakte>) {
    return callApi('PATCH', `/apps/${APP_IDS.KONTAKTE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'kontakte') });
  }
  static async deleteKontakteEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.KONTAKTE}/records/${id}`);
  }
