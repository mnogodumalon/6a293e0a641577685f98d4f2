...
import { formEnhancements } from '@/config/form-enhancements/Kontakte';
import { evalComputed } from '@/config/form-enhancements/types';

export default function KontakteDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [record, setRecord] = useState<Kontakte | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const list = await LivingAppsService.getKontakte();
  ...
  async function handleUpdate(fields: Kontakte['fields']) {
    if (!record) return;
    await LivingAppsService.updateKontakteEntry(record.record_id, fields);
    await loadData();
    setEditing(false);
  }
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteKontakteEntry(record.record_id);
    setDeleteOpen(false);
    navigate('/kontakte');
  }
  ...
  if (!record) {
    return (
      <RecordViewEmpty
        ...
        action={
          <Button variant="ghost" onClick={() => navigate('/kontakte')}>
  ...
  return (
    <RecordView
      onBack={() => navigate('/kontakte')}
      onEdit={() => setEditing(true)}
      backLabel="Zurück"
      editLabel="Bearbeiten"
    >
      <RecordHeader title={record.fields.nachname ?? 'Kontakte'} />
      ...
      <RecordSection title="Details" cols={2}>
        ...
        <RecordField label="Mobilnummer" value={record.fields.mobil} format="text" />
      ...
      <RecordAttachments appId={APP_IDS.KONTAKTE} recordId={record.record_id} />
