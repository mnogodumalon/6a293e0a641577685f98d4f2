...
import { KontakteViewDialog } from '@/components/dialogs/KontakteViewDialog';
...
const KONTAKTE_FIELDS = [
  { key: 'nachname', label: 'Nachname', type: 'string/text' },
  { key: 'geburtsdatum', label: 'Geburtsdatum', type: 'date/date' },
  { key: 'kategorie', label: 'Kategorie', type: 'lookup/select', options: [{ key: 'privat', label: 'Privat' }, { key: 'geschaeftlich', label: 'Geschäftlich' }, { key: 'familie', label: 'Familie' }, { key: 'freunde', label: 'Freunde' }, { key: 'sonstiges', label: 'Sonstiges' }] },
...
const ENTITY_TABS = [
  { key: 'kontakte', label: 'Kontakte', pascal: 'Kontakte' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('kontakte');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'kontakte': return (data as any).kontakte as Kontakte[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'kontakte': return KONTAKTE_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'kontakte': return {
        create: (fields: any) => LivingAppsService.createKontakteEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateKontakteEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteKontakteEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'kontakte' || dialogState?.entity === 'kontakte') && (
        <KontakteDialog
          open={createEntity === 'kontakte' || dialogState?.entity === 'kontakte'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'kontakte' ? handleUpdate : (fields: any) => handleCreate('kontakte', fields)}
          defaultValues={dialogState?.entity === 'kontakte' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Kontakte']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Kontakte']}
        />
      )}
      {viewState?.entity === 'kontakte' && (
        <KontakteViewDialog
          open={viewState?.entity === 'kontakte'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'kontakte', record: r }); }}
        />
      )}
